 ...
 */
public class ActivateSchool implements NetworkContants {
    private MaterialDialog.Builder builder;
    private MaterialDialog dialog;

    public void activateSchool(final Context context, SchoolDTO schoolDTO) throws Exception {

        builder = GetProgressBar.get(context);
        String url = IP + ACTIVATE;
        builder.content("Loading...");
        dialog = builder.show();
        final Gson gson = new Gson();

        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.POST, url, new JSONObject(gson.toJson(schoolDTO)), new Response.Listener<JSONObject>() {
